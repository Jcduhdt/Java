 ...
 *        这样的问题都可以针对性的处理
 *      2.编译时不检测异常（运行时异常）：就是Exception中的RuntimeException子体系和其子类
 *        这种问题的发生，无法让功能继续，运算无法进行，更多是因为调用者的原因导致的，或者引发了内部状态的改变导致的
 *        这种问题一般不处理，直接编译通过，在运行时，让调用者调用程序时强制停止，让调用者对代码进行调试
 * 所以自定义异常时，要么继承Exception，要么继承RuntimeException
 *
 * thorws 和 throw 的区别：
 *      1.throws使用在函数上
 *        thorw使用在函数内
 *      2.thorws抛出的是异常类，可以抛出多个，用逗号隔开
 *        thorw抛出的是异常对象
 */
...
class Demo
{
    public int method(int[] arr,int index)throws FuShuIndexException
    {
        ...
        if (index>=arr.length)
        {
            throw new ArrayIndexOutOfBoundsException("数组角标越界了，哥们，你是不是疯了");
        }
        if (index<0)
        {
