 ...
 *           scpoe=""  init-method="" destory-method="">
 ...
 *          属性：
 *              value：用于指定bean的id。当我们不写时，他的默认值时当前类名。且首字母小写
 *      @Controller：一般用在表现层
 *      @Service：一般用在业务层
 *      @Repository：一般用在持久层
 *      以上三个注解他们的作用和属性与Component时一模一样的，
 *      他们三个是spring框架为我们提供的明确的三层使用的注解，是我们的三层对象更加清晰
 * 用于注入数据的
 ...
 *      @Qualifier:
 *          作用:在按照类中注入的基础之上再按照名称注入。它给类成员注入时不能单独使用。但是在给方法参数注入时可以
 *          属性：
 *              value：用于指定注入bean的id
 *      @Recource:
 ...
 *          作用：用于指定bean的作用范围
 *          属性：
 *              value：指定范围的取值。常用取值：singleton/prototype 分别对应单例多例
 * 和生命周期相关的
