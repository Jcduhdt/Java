...
public class UpdateCustomerFrame extends CustomerFrame {
    ...
    private JCheckBox chckbxNewCheckBox;					// 隐藏失效信息多选框
    ...
    private void init() {
        ...
        getContentPane().add(southPanel, BorderLayout.SOUTH);// 将次面板放到主容器的南部
    ...
    private void setTableModelSource(byte type) {
        if (tableModel == null) {							// 如果表格数据模型不是空的
            // 定义表格列名数组
            String[] columnNames = { "编号", "办公电话", "移动电话",
                    "电子邮箱", "QQ", "是否有效" };
        ...
        List<Communication>usableList = null;				// 创建用于展示的通信信息集合
        ...
        int comCount = usableList.size();					// 获取通信信息集合长度
        String[] tableValues = new String[6];				// 创建保存一行通信信息数据的数组
        for (int i = 0; i<comCount; i++) {					// 遍历通信信息集合
            Communication com = usableList.get(i);			// 获取集合中的通信信息对象
     ...
     */
    public void dispose(){
        super.dispose();//调用父类窗体销毁方法  window类
    ...
    private void addAction() {
        ...
        addRowBtn.addActionListener(new ActionListener() {				// 添加行按钮
            public void actionPerformed(ActionEvent e) {		// 点击时
                Communication addCom = new Communication();	// 创建空通信信息
        ...
        delRowBtn.addActionListener(new ActionListener() {				// 删除行按钮
            public void actionPerformed(ActionEvent e) {		// 点击时
                ...
                if (selectedRow != -1) { 						// 判断是否存在被选中行
                    ...
                    Communication del = new Communication();	// 创建空通信信息
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (checkInfo()) {
                    ...
                    for (int i = 0; i < rowcount; i++) {                    // 遍历所有行
                        String id = (String) table.getValueAt(i, 0);    // 通信信息ID
                        ...
                        updateCom.setId(Integer.parseInt(id));            // 设置通信信息编号
                        updateCom.setAvailable(available);            // 设置通信信息是否有效
                        dao.updateCommunication(updateCom, MainFrame.getUser());// 更新通信信息
                    }// for结束
                    // 弹出对话框
    ...
    private boolean checkInfo() {
        ...
        for (int i = 0; i<rowcount; i++) {				// 遍历表格中所有行
            ...
            if (!(email_str.matches("\\w+@\\w+(\\.\\w{2,3})*\\.\\w{2,3}") || email_str.equals(""))) {
                result = false;						// 校验结果为不通过
                sb.append("电子邮箱格式错误！\n");		// 记录错误日志
            } // if结束
            // 如果QQ号码不为空，且不是有效QQ号码
